...
export default function App() {
  ...
  const decrement = () => {
    switch (type) {
      ...
      case "month":
        ...
        break;
      case "year":
        setDatepickerHeaderDate((prev) => subYears(prev, 11));
        break;
    }
  };
  ...
  const increment = () => {
    switch (type) {
      ...
      case "month":
        ...
        break;
      case "year":
        setDatepickerHeaderDate((prev) => addYears(prev, 11));
        break;
    }
  };
  ...
  const isSelectYear = (year: number) =>
    isEqual(
      new Date(selectedDate.getFullYear(), year, selectedDate.getDate()),
      selectedDate
    );

  const generateArrayOfYears = (year: number) => {
    let max = year;
    let min = max - 11;
    let years = [];
